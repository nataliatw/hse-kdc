...
export const downloadInspectionJSON = () => {
  const { startDate, endDate, filteredDownloadData } = useInspectionStore.getState();
  ...
  const filename = `inspection_data${filenameSuffix}.json`;
...
export const downloadInspectionCSV = () => {
  const { startDate, endDate, filteredDownloadData } = useInspectionStore.getState();
  const dataToDownload = filteredDownloadData;
  ...
  const filenameSuffix = (startDate && endDate) ? "_Filtered" : "_Full";
  const filename = `inspection_data${filenameSuffix}.csv`;
...
export const downloadInspectionExcel = () => {
  const { filteredDownloadData } = useInspectionStore.getState();
  const dataToDownload = filteredDownloadData;
  ...
  try {
    ...
    XLSX.writeFile(workbook, "Inspection_Details.xlsx");
